...
/*
sleep(2);
echo ('{handler: function(){alert(this);this.find("input").val(888);}}');
exit;
*/
...
if(!isEmptyArr($_POST) && $_POST['task'] == 'dental-examination')
{
	...
	if(isEmptyArr($errorsArray))
	{
		...
		insertItem($_SQL_TABLE['site_form_dental_examination'], $strColumns, $strValues);
	    ...
	    $mailTemplateQuery->from(array('mt' => $_SQL_TABLE['mail_template']),
	        ...
	        ->where('mt.mail_template_id IN (4,7)');
        ...
        $bodyUser = str_replace($formularDataSearch, $formularDataReplace, $mailTemplate[7]['body']);
        ...
        $mail->setBodyHtml($bodyUser)
			...
			->setSubject($mailTemplate[7]['subject']);
        $mail->send();

		echo ('{handler: function(){
					alert("'.$language['site']['dentalExaminationRequestSuccessfullySent'].'");
		...
		//echo ('{handler: function(){alert("'.$language['site']['dentalExaminationRequestSuccessfullySent'].'");}}');
		//exit;
	}
	else
	{
