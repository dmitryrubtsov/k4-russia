...
{foreach from=$Field.titleValues item='title' key='key1'}
{foreach from=$Field.values[$key1] item="curr" key="key"}
...
{foreach from=$Field.titleValues item='title' key='key'}
{$Field.separator|unescape}<strong>{$title}</strong>{$Field.separator|unescape}
{html_checkboxes name=$Field.name options=$Field.values[$key] selected=$Field.selected separator=$Field.separator|unescape other=$Field.other|unescape}
{/foreach}
