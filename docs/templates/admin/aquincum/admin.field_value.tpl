{if $Field.defaultValue}
	{if $Field.colorfield eq 'y' && $Field.colorcode}
		<span style="color:{$Field.colorcode}; font-weight:bold;">
			{$Field.defaultValue}
		</span>
	{else}
		{$Field.defaultValue}
...
{else}
	...
	{if $Field.isLink eq 'y'}
		<a href="{if $Field.linkURL eq ''}{$Field.value|unescape}{else}{$Field.linkURL|unescape}{/if}"{if $Field.className neq ''} class="{$Field.className}"{/if} target="{$Field.linkTarget}">
	{elseif $Field.className neq ''}
...
{/if}
{if $Field.textUnderField && $FLAGS.ListingTab neq $TRUE}
