...
function smarty_function_gen_fields($params, &$smarty)
{
    ...
    {
		if((($flag == __TRUE && isBlank($smarty->get_template_vars('showAddNewForm'))) || $field['useInAddForm'] == 'y') && $field['type'] != 'hidden')
		{
			/*$str .= '
					<tr>
						<td '.(($field['position'] == 'vertical') ? 'class="doubletitle" colspan="2"' : 'class="title"').'>
							'.$field['title'].((!isBlank($field['required'])) ? '&nbsp;
							<span class="required">{$Config.AdminReqFieldSymbol}</span>' : '').'
						</td>'.(($field['position'] == 'vertical') ? '</tr><tr>' : '').'
						<td '.(($field['position'] == 'vertical') ? 'class="doublefield" colspan="2"' : 'class="field"').'>
							{include file="admin.field_'.$field['type'].'.tpl" Field=`$ConfFields.'.$name.'`}'.((!isBlank($field['editable']) && $name == 'linkname') ? '&nbsp;&nbsp;<a href="#" onclick="showMessage(\''.$name.'\');return false;">{$lang.admin.edit}</a>' : '').'</td>
					</tr>';  */
			$str .= '<div class="formRow">
							<div class="grid'.(($field['useInAddFormLocation'] == 'full') ? 13 : 3).'"><label>'.$field['title'].':'.(($field['required']) ? '<span class="req">*</span>' : '').'</label></div>
							<div class="grid'.(($field['useInAddFormLocation'] == 'full') ? 13 : 9).'">{include file="admin.field_'.$field['type'].'.tpl" Field=`$ConfFields.'.$name.'`}'.((!isBlank($field['editable']) && $name == 'linkname') ? '&nbsp;&nbsp;<a href="#" onclick="showMessage(\''.$name.'\');return false;">{$lang.admin.edit}</a>' : '').'</div>
					</div>';
